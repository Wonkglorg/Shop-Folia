...
public class ShopMessage {
    ...
    public static TextComponent replacePlaceholder(String placeholder, PlaceholderContext context) {
        plugin.logger().spam("[ShopMessage.replacePlaceholder] Attempting to replace placeholder: " + placeholder + " " + context);
        Function<PlaceholderContext, TextComponent> valueFunction = placeholders.get(placeholder.toLowerCase());
        if (valueFunction != null) {
            try {
                plugin.logger().spam("[ShopMessage.replacePlaceholder]     Running placeholder function... " + placeholder);
                TextComponent message = valueFunction.apply(context);
                if (message != null) {
                    plugin.logger().trace("[ShopMessage.replacePlaceholder]  *** placeholder " + placeholder + "  value: " + message);
                    return message;
                }
            } catch (Error | Exception e) {
                // Log the exception
               plugin.logger().severe("Error replacing placeholder " + placeholder + ": " + e.getMessage());
            }
        }
        // If placeholder not found, remove the placeholder and just return an empty string
        plugin.logger().spam("[ShopMessage.replacePlaceholder] *** returning empty string, unable to get function to replace placeholder: " + placeholder);
    ...
    public static TextComponent format(String message, PlaceholderContext context) {
        ...
        plugin.logger().spam("[ShopMessage] pre-format: " + ChatColor.translateAlternateColorCodes('&', message), true);
        ...
        for (String part : parts) {
            plugin.logger().spam("\n\n");
            plugin.logger().trace("[ShopMessage.format] part: " + part);
                ...
                try {
                    ...
                    if (newColor != null) {
                        if (newColor == ChatColor.BOLD) isBold = true;
                            ...
                            plugin.logger().hyper("[ShopMessage.format]     matched RESET color code: " + part);
                    ...
                    plugin.logger().hyper("[ShopMessage.format]     matched COLOR_CODE_REGEX: " + part);
                    plugin.logger().hyper("[ShopMessage.format]     newColor: " + newColor.toString());
                    plugin.logger().hyper("[ShopMessage.format] *** skipping to next part: " + newColor.getName().toUpperCase());
                    plugin.logger().hyper("[ShopMessage.format]     isBold: " + isBold + " isItalic: " + isItalic + " isStrikethrough: " + isStrikethrough + " isUnderlined: " + isUnderlined + " isObfuscated: " + isObfuscated);
                    continue; // Don't add this text to the message, just go to the next part
                } catch (Exception e) {
                    plugin.logger().hyper("[ShopMessage.format] XXX unknown color code! Going to add this as a normal string! " + part);
                }
            }
            ...
            if (part.matches(PLACEHOLDER_REGEX)) {
                plugin.logger().hyper("[ShopMessage.format]     matched PLACEHOLDER_REGEX: " + part);
                plugin.logger().hyper("[ShopMessage.format]     is part placeholder? " + (placeholders.get(part) != null));
                if (placeholders.get(part) != null) {
                    plugin.logger().hyper("[ShopMessage.format]     replacing placeholder... " + part);
                    ...
                    if (partComponent.getColor() != latestColor && partComponent.getColor() != null && partComponent.getColor() != ChatColor.WHITE) { 
                        plugin.logger().hyper("[ShopMessage.format]     getting latestColor from partComponent: " + partComponent.getColor().getName().toUpperCase());
            ...
            // Set the color
            if (latestColor != null) {
                plugin.logger().hyper("[ShopMessage.format]     setting part color to: " + latestColor.getName().toUpperCase());
            ...
            addedText = true;
            plugin.logger().hyper("[ShopMessage.format] *** add part TextComponent to main message: " + partComponent);
        }
        ...
        plugin.logger().spam("[ShopMessage] postFormat: " + formattedMessage.toLegacyText(), true);
    ...
    public static void sendMessage(String message, Player player, PlaceholderContext context) {
        ...
        plugin.logger().debug("Sent msg to player " + player.getName() + ": " + fancyMessage.toLegacyText(), true);
        try {
            player.spigot().sendMessage(fancyMessage);
            return;
        } catch (JsonParseException e) {
            plugin.logger().warning("Possible NBT error while sending message to player, Item Hover events will now be disabled! Details: " + e.getMessage());
            plugin.logger().debug("Error details: ", e);
            // disableItemHover = true;
        } catch (Exception | Error e) {
            plugin.logger().warning("Error sending message to player: " + e.getMessage());
            plugin.logger().debug("Error details: ", e);
        }
        ...
        try {
            ...
            plugin.logger().warning("Sent legacy text message to player as backup, removed hover/click events");
        } catch (Error | Exception e) {
            plugin.logger().debug("Error sending message to player", e);
        }
    }
