...
public class Transaction{
	...
	public TransactionError verify() {
		...
		if(this.error == TransactionError.CANCELLED){
			return this.error;
		}
		...
		if(!this.buyer.hasRoomForItem(itemToBeAdded)){
			...
			if(this.buyer.isPlayer()){
				return this.setError(TransactionError.INVENTORY_FULL_PLAYER);
			} else {
	...
	public TransactionError execute() {
		...
		if(this.verify() != TransactionError.NONE){
			return this.error;
		}
		...
		if(shop.getType() == ShopType.GAMBLE){
			...
			if(shop instanceof GambleShop){
				((GambleShop) shop).shuffleGambleItem(player);
			}
			// Exit early
			return this.setError(TransactionError.NONE);
		}
	...
	public Map<ItemStack, Integer> getItems() {
		...
		if(shop.getSecondaryItemStack() != null){
			items.put(shop.getSecondaryItemStack(), (int) this.price);
		}
		return items;
	}
	...
	public TransactionError setError(TransactionError error) {
		if(TX_DEBUG_LOGGING){
			System.out.println("TX setError: TransactionError." + error.toString().toUpperCase());
		}
		this.error = error;
		return this.error;
	}
