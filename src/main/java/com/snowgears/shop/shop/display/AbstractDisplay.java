...
public abstract class AbstractDisplay{
	...
	public void spawn(Player player) {
		...
		if(shop == null || shop.getItemStack() == null || shop.getContainerLocation() == null){
			return;
		}
		...
		//one display entity on the chest
			...
			switch(displayType) {
				...
				case ITEM:
					...
					break;
				case LARGE_ITEM:
					ArmorStandData armorStandData = DisplayUtil.getArmorStandData(item,
							shop.getContainerLocation().clone().add(0, 1, 0),
							shop.getFacing(),
							false);
					spawnArmorStandPacket(player, armorStandData, null);
					break;
	...
	public AbstractShop getShop() {
		return Shop.getPlugin().getShopmanager().getShopBySign(this.shopSignLocation);
	}
	...
	protected Iterator<Integer> getDisplayEntityIDIterator(Player player, boolean onlyDisplayTags) {
		...
		if(onlyDisplayTags){
			...
			if(player == null){
				...
				this.entityIDs.computeIfAbsent(player.getUniqueId(), k -> new ArrayList<>());
