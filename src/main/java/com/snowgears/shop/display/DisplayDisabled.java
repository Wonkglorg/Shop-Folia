...
public class DisplayDisabled extends AbstractDisplay {
    ...
    public DisplayDisabled(Location shopSignLocation) {
        ...
        if (!Shop.loggedDisplayDisabledWarning) {
            ...
            Shop.getPlugin().logger().warning("[DisplayDisabled] This could mean there was an error with the server running an unsupported version or unsupported server software.");
            Shop.getPlugin().logger().warning("[DisplayDisabled] Shop will attempt to function without display features, but some features may not work as expected.");
    ...
    protected void spawnItemPacket(Player player, ItemStack is, Location location) {
        Shop.getPlugin().logger().debug("Display is disabled, item packet not sent");
    }
    ...
    protected void spawnItemFramePacket(Player player, ItemStack is, Location location, BlockFace facing, boolean isGlowing){
        Shop.getPlugin().logger().debug("Display is disabled, item frame packet not sent");
    }

    @Override
    public void removeDisplayEntities(Player player, boolean onlyDisplayTags) {
        Shop.getPlugin().logger().debug("Display is disabled, removeDisplayEntities not called");
    }

    @Override
    public String getItemNameNMS(ItemStack item) {
        Shop.getPlugin().logger().debug("Display is disabled, getItemNameNMS not called, returning empty string");
